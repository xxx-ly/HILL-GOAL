...
(function ($) {
  var freshGridClass = {
      ...
      if ("top" == bgLayerData.edge) {
        if ("up" == bgLayerData.direction) {
          calculateSlantFunction = function (thisElWidth, thisElHeight) {
            $bgLayer.css({
              "border-top":
                Math.tan(bgLayerData.angle * (Math.PI / 180)) * thisElWidth +
                "px solid " +
                bgLayerData.color,
      ...
      if ("right" == bgLayerData.edge) {
        if ("up" == bgLayerData.direction) {
          calculateSlantFunction = function (thisElWidth, thisElHeight) {
            $bgLayer.css({
              "border-right":
                Math.tan(bgLayerData.angle * (Math.PI / 180)) * thisElHeight +
                "px solid " +
                bgLayerData.color,
    ...
    recalculateVideoLayerPlayOrStop: function (
      bgLayerData,
      $bgLayer,
      $element,
      dimensionsHasBeenChanged
    ) {
      var self = this;
      /*----------------------------------------------------------*/
    ...
    recalculateVideoLayer: function (
      bgLayerData,
      $bgLayer,
      $element,
      dimensionsHasBeenChanged
      ...
      (function () {
        ...
        if (
          thisElHeight >
          (thisElWidth / bgLayerData.width) * bgLayerData.height
        ) {
          newVideoWidth =
            (thisElHeight / bgLayerData.height) * bgLayerData.width * 1.005;
          ...
          newVideoHeight =
            (thisElWidth / bgLayerData.width) * bgLayerData.height * 1.005;
    ...
    mainLoop: function () {
      ...
      var loop = function () {
        if (
          self.fgWowHasBeenInit == false &&
          $(".animsition-loading").size() == 0
        ) {
          ...
          if (layerTypes) {
            if (layerTypes.indexOf("video") != -1) {
              $this.find(".fg-bg-type-video").each(function () {
                ...
                self.recalculateVideoLayerPlayOrStop(
                  bgLayerData,
                  $bgLayer,
                  $element,
                  hasBeenResized
    ...
    recalculateSlantLayer: function (
      bgLayerData,
      $bgLayer,
      $element,
      dimensionsHasBeenChanged
    ...
    recalculateParallaxLayer: function (
      bgLayerData,
      $bgLayer,
      $element,
      dimensionsHasBeenChanged
      ...
      if (this.breakpoint >= 3) {
        ...
        if ("auto" == bgLayerData.size) {
          ...
          calcBgPosX =
            thisElLeft +
            (thisElWidth - bgLayerData.width) * (bgLayerData.offset_h / 100) +
            "px";

          calcBgPosY =
            ((thisElTop - winScrollTop) * bgLayerData.speed) / 100 +
            (thisElHeight - bgLayerData.height) * (bgLayerData.offset_v / 100) +
            "px";
          ...
          if (
            newBgHeight <
            this.winHeight -
              (this.winHeight - thisElHeight) * (bgLayerData.speed / 100)
          ) {
            newBgHeight =
              this.winHeight -
              (this.winHeight - thisElHeight) * (bgLayerData.speed / 100);
          ...
          calcBgPosY =
            (thisElTop - winScrollTop) * (bgLayerData.speed / 100) + "px";
    ...
    recalcMatchColumnsHeight: function () {
      ...
      this.$matchColumns.each(function () {
        ...
        $cols.each(function () {
          ...
          /* BASIC COLUMN HEIGHT
                     /*----------------------------------------------------------*/
          var basicColumnHeight = $col
            .find(".fg-match-column-inside-wrapper:first")
            .height();
        ...
        if (matchThisBp) {
          $cols
            .filter(":not(.fg-col-not-match)")
            .css("height", biggestColumnHeight);
        }
      });
    },
    ...
    recalculateMinHeight: function () {
      ...
      this.$minHeight.each(function () {
        ...
        if (data["breakpoint_" + breakpoint] != undefined) {
          ...
          var final_height =
            self.winHeight * (new_height / 100) + new_offset + 0.2; // 0.2 is a hack to compensate for "integer only" output from matchHeight
      ...
      $bgVideo.each(function () {
        ...
        if ("youtube" == data.variant) {
          ...
          if (url) {
            ...
            $layer.append(
              '<div class="fg-youtube-iframe" data-videoId="' +
                videoID[1] +
                '"></div>'
            );
          }
          ...
          var url = data.url;
          $layer.append(
            '<video class="fg-bg-html-video-frame" autoplay loop ' +
              mutedAttr +
              ' playsinline><source type="video/mp4" src="' +
              url +
              '"></source></video>'
          );
        }
    ...
    initYoutubeVideos: function () {
      ...
      window.onYouTubeIframeAPIReady = function () {
        if (
          ...
          this.initYoutubeVideosCounter < 4
        ) {
          self.initYoutubeVideosCounter++;
          setTimeout(function () {
            self.initYoutubeVideos();
          }, 300);
          return;
        }
